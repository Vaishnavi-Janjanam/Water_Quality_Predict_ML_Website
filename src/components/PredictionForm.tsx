...
export const PredictionForm = ({ onPrediction }: PredictionFormProps) => {
  ...
  const inputFields = [
    ...
    { key: 'dissolvedOxygen', label: 'Dissolved Oxygen', unit: 'mg/L', icon: '💨' },
    { key: 'pH', label: 'pH Level', unit: '', icon: '⚗️' },
    { key: 'conductivity', label: 'Conductivity', unit: 'µS/cm', icon: '⚡' },
    { key: 'bod', label: 'BOD', unit: 'mg/L', icon: '🧪' },
    { key: 'nitrate', label: 'Nitrate', unit: 'mg/L', icon: '🧬' },
    { key: 'fecalColiform', label: 'Fecal Coliform', unit: 'MPN/100mL', icon: '🦠' }
  ];
    ...
    <Card className="water-card bg-white/95 backdrop-blur-sm">
      <CardHeader className="text-center">
        <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <FlaskConical className="h-6 w-6 text-accent animate-pulse" />
        </div>
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {inputFields.map(({ key, label, unit, icon }) => (
